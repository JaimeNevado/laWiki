...
async def create_article_for_wiki(wiki_id: str, article_request: Article):
    # Add the wiki_id to the article request data
    article_data = article_request.dict()
    article_data["wiki_id"] = wiki_id

    # Send a POST request to the articles microservice to create the article
    response = requests.post("http://localhost:7000/articles/", json=article_data)
